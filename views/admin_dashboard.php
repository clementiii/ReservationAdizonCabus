...
if (!isset($reservations)) {
    $reservations = []; // Ensure it's an array even if no data passed
                ...
                <tbody>
                    <?php foreach ($reservations as $res): ?>
                        <tr>
                            ...
                            <td style="border: 1px solid #ddd; padding: 8px;">
                                <?php echo htmlspecialchars(ucfirst($res['status'])); ?></td>
                            <td style="border: 1px solid #ddd; padding: 8px;">
                                <?php echo htmlspecialchars(date('Y-m-d H:i', strtotime($res['created_at']))); ?></td>
                            <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">
                                <!-- Add links/buttons for actions like Confirm, Cancel, Delete -->
                                <!-- Example (needs corresponding controller actions): -->
                                <!-- <a href="index.php?action=adminConfirmReservation&id=<?php echo $res['id']; ?>">Confirm</a> -->
                                <!-- <a href="index.php?action=adminCancelReservation&id=<?php echo $res['id']; ?>">Cancel</a> -->
                                <!-- <a href="index.php?action=adminDeleteReservation&id=<?php echo $res['id']; ?>" onclick="return confirm('Are you sure?');">Delete</a> -->
                                <span style="color: #999;">Actions TBD</span>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
